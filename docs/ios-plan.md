...
**与设计稿的取舍（需设计拍板）：** 设置总览「允许从其他 App 粘贴」右值显示「系统设置」而非「询问」（iOS 不提供读取该授权的 API）；04b 没有「打开操作按钮设置」按钮（无公开深链）；设置总览「Copyo 键盘」右值同样显示「系统设置」而非设计稿的「未启用」（iOS 既不提供读取「键盘是否已添加」的 API，也不提供读取「允许完全访问」的 API，主应用没有任何办法说出真状态）；04e 底部按钮叫「打开 Copyo 的系统设置」而非「打开键盘设置」（唯一的公开深链 `openSettingsURLString` 只到 Copyo 自己那一格，那里有「键盘」行与「允许完全访问」开关即第 3 步，而第 1、2 步要去的「设置 › 通用 › 键盘 › 键盘」跳不过去）；隐私说明是外链而非二级页；颜色详情的三个色值胶囊在 440pt 宽上折成两行；引导页插图符号偏下约 14pt；iOS 26 系统返回按钮不带「历史」文字。
...
- [x] 键盘扩展：代码完成（2026-09-21，见 3.6）。**刻意不随 iOS 首版一起发**——target 在工程里、CI 照编，但不在包里
...
索引等于把它们送进主屏下拉、锁屏搜索和 Siri 建议，只能由用户自己按下，设置页文案也把这件事说破了。
关掉时清空索引：「以后不再加」不等于「现在看不到」。

标识符用编码后的 `PersistentIdentifier`，**没有**给 `ClipItem` 加搜索列——加一列就是一次
CloudKit schema 变更，而 Production schema 已部署、Mac 1.0 正拿它在审核中。代价是标识只在同一个
store 文件内稳定，重装 / 重建库由 store 令牌兜住。

##### 踩到的坑：`JSONEncoder` 编 `PersistentIdentifier` 必须加 `.sortedKeys`

`PersistentIdentifier` 把五个子键编进一个字典，而 **Swift 的字典遍历顺序按进程随机化**。
同一个进程里每次编码一致，所以单进程自测完全看不出问题；**换一次进程就换一种顺序**——
实测连开六次进程得到六种不同顺序，加上 `.sortedKeys` 后六次完全一致。

不加的后果是必然的，而且三条都不报错：

1. **撤索引永远失效**。索引是上次启动写的、删除在这次启动发起，两个字符串对不上，
   `deleteSearchableItems(withIdentifiers:)` 一条也删不掉——用户删掉的验证码会继续留在锁屏搜索里，
   直到 30 天后自己过期。这正好是设置页承诺不会发生的事。
2. **重复条目**。重新索引写成新 `uniqueIdentifier` 而不是覆盖，一条剪贴板在系统搜索里出现好几遍。
3. **每次对账全量重建**。账本里全是上一代的字符串，比对出来「整库新增、整本账撤销」。

**以后任何把 `PersistentIdentifier` 编码成字符串当键用的地方，都要显式 `.sortedKeys`。**

其余几条：`enforceHistoryLimit` 改成先收集再删（`save` 之后 `persistentModelID` 取不回来，
那些条目就成了点不开的孤儿），并返回被挤掉的标识符；去重命中的 `.refreshed` 分支也要重新索引
...
#### 3.6 键盘扩展（2026-09-21，代码完成但不随首版发布）

`CopyoKeyboard` target，约 2000 行。能编译、能打字、4.4.1 该有的都有了，但**刻意不随 iOS
首个版本（1.1.0）一起发**。

##### 为什么先摘出来

- 它要申请「允许完全访问」——这是普通用户心理门槛最高的权限，审核员也最警惕
- 它把整个 iOS 版拖进审核指南 4.4.1 的审视范围，而 iOS 版**本身还有一整条从未在签名真机上
  跑通过的链路**（App Group 共库、CloudKit、一键保存，见 3.2 的真机验证清单）
- 第一次提审的原则是尽量缩小审核面。Mac 1.0 已经吃过一轮 2.4.5 和一轮 2.1

##### 具体是怎么摘的

从 `Copyo iOS` 上摘掉两处引用，**target 本身一行都没动**：

- 「Embed Foundation Extensions」拷贝阶段里那条（`AB…4F`，连同它的 `PBXBuildFile`）
- target dependency（`AB…58`，连同只有它用的 `PBXContainerItemProxy` `AB…50`）

1.1 要发的时候，把这三个对象照 `CopyoWidgets` 的同名对象再抄一遍加回去即可
（`AB…24` / `AB…3C` / `AB…26` 就是它们的样板）。

摘除带来一个必须一起堵的洞：**键盘不再是宿主的依赖，编 `Copyo iOS` 就不会再碰这 2000 行**。
所以补了两样：

- `Copyo.xcodeproj/xcshareddata/xcschemes/CopyoKeyboard.xcscheme`（签进仓库）。另外两个扩展
  没有 scheme 是因为它们跟着宿主被编；键盘现在不跟着编了，而靠 Xcode 自动生成 scheme
  是一项**偏好设置**、不是工程的一部分，干净检出上不保证存在
- CI 里一条独立的「编译 CopyoKeyboard」步骤，同样校验 `.appex` 真的产出了

应用内那一行「Copyo 键盘」与 04e 引导页按 `KeyboardExtension.isBundled` 判断出不出现——
读的是包里到底有没有 `CopyoKeyboard.appex`，不是一个手动开关。开关要靠人记得翻，两种忘法
都难被发现；按 appex 判断是自洽的，哪天把 embed 挂回去那一行自己就回来了。

##### 4.4.1：设计稿画的那一版会被拒

设计 07 的底排是 地球 / `↑` / 空格 / 删除 / 换行，**一个字符都打不出来**；07b 未授权态更是
什么都打不了。而 4.4.1 要求键盘「Provide keyboard input functionality (e.g. typed characters)」
**并且**「Remain functional without full network access and without requiring full access」。
照着设计稿实现就是一次必然的拒绝。

所以：

- `↑` 改成写「去处」的切换键（卡片条上写 `ABC`、字母面上写 `123`）。4.4.1 明文禁止把键盘按键
  挪作他用，一颗长得像上档键、按下去却翻一整面键盘的键正是那条禁令说的事
- 加了一整面能打字的三排键（`LetterPlane`），**未授权态也够得着**——那是这块键盘没开完全访问
  时仍然可用的全部理由
- 范围限定拉丁字母。做中文输入法不在这一版的讨论范围内，于是键盘自己的搜索框只能打拉丁字母，
  而它要搜的内容往往是中文——**这是一个真实的体验缺口，需要产品决定**，不是可以糊过去的细节

##### 两处设计稿做不到的，已记进 design-spec 第八节待确认

1. 「长按卡片预览」不能像设计稿那样浮在键盘上方：Apple 的自定义键盘规则禁止在键盘主视图的
   上边缘之外绘制内容，而向上弹出的 `.contextMenu(preview:)` 正是那件事。改成盖在 330pt 之内
2. 07b 的「打开设置」按钮没有公开 API 可用：`APPLICATION_EXTENSION_API_ONLY` 下够不着
   `UIApplication`，`NSExtensionContext.open` 不支持键盘扩展点，走响应链那套是未公开做法且
   被拒过。改成纯指引文案，真正的深链放在应用内的 04e 页——那里 `openSettingsURLString` 能用

##### 这一轮修掉的两个缺陷

- **符号面第三排溢出**。`keyWidth` 是按上面那排 10 颗算的，而底排两端各有一颗 46pt 功能位；
  9 颗字符键在 440pt 机身上要 476pt，而可用宽只有 424。`VStack` 没有 `clipped()`，所以那一排
  居中后从两边各探出去二三十点，**而 UIKit 的命中测试不越过父视图边界**——最外侧那两颗
  画得出来、点不到。底排收回注释里本来就写着的 5 颗（`. , ? ! '`），并加了一道按本排实际
  占用重算键宽的闸门，放不下就整排一起缩，免得再悄悄溢出一次
- **三处假注释**声称「根视图的 body 一个字都不读 `query`」。实际上 `clipsArea` 读了两次。
  这条性能保证真正靠的是「卡片条那一面改不了查询」（`isEditingQuery` 要求有字符键），
  不是「根视图不读」。注释已改成记录真正的前提，并写明什么改动会破坏它

##### 还没验的

键盘**一次都没在真机上跑过**，而它恰恰是最依赖真机的一个：没有「完全访问」就读不到 App Group，
而那个权限只有签名安装之后才存在。真机清单见 3.2。
